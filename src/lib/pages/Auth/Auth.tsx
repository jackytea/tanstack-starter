      ...
      <>
        {socialLoginButtons.map((socialLoginButton) => (
          <Button
            ...
            onClick={() =>
              handleSocialLogin(socialLoginButton.provider as AuthProvider)
            }
          >
            <div className="flex items-center justify-center gap-2">
              {socialLoginButton.icon}
              <span>
                {localize.loginWith({ authProvider: socialLoginButton.text })}
              </span>
            </div>
          </Button>
        ))}
          ...
          <Card>
            <CardHeader className="text-center">
              <CardTitle className="text-xl">
                {localize.welcomeTo({ appName: APP.NAME })}
              </CardTitle>
              <CardDescription>
                {user
                  ? localize.youAreLoggedIn()
                  : localize.loginToGetStarted()}
              </CardDescription>
            </CardHeader>
            <CardContent>
